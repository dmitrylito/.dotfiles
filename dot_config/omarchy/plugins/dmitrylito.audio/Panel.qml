...
Panel {
  ...
  // "header" is a virtual section for the real master audio mute toggle.
  readonly property bool headerHasCursor: cursorActive && focusSection === "header"
  // Quickshell can retain unbound PipeWire nodes after USB reconnects. Read
  // the master state from the audio server instead of trusting those objects.
  readonly property bool hasOutput: !!(volumeSink && volumeSink.audio)
  readonly property bool hasInput: !!(source && source.audio)
  property bool masterAudible: false
  readonly property string toggleHint: masterAudible ? "Mute" : "Unmute"
  readonly property string masterControlPath: Quickshell.env("HOME") + "/.config/omarchy/plugins/dmitrylito.audio/audio-master-control"
  ...
  function toggleOutputMute() {
    if (!sinkToggleProc.running) sinkToggleProc.running = true
  }

  function toggleInputMute() {
    if (!sourceToggleProc.running) sourceToggleProc.running = true
  }

  function updateMasterState(raw) {
    var fields = String(raw).trim().split(/\s+/)
    if (fields.length < 3) return
    masterAudible = fields[0] === "1"
    outputMuted = fields[1] !== "1"
    inputMuted = fields[2] !== "1"
  }

  function refreshMasterState() {
    if (!masterStateProc.running) masterStateProc.running = true
  }

  // Toggle the real default sink and every live node matching the default
  // source name. This handles duplicate nodes left by USB reconnects.
  function toggleAllMuted() {
    if (!masterToggleProc.running) masterToggleProc.running = true
  }
  ...
  Process {
    id: masterStateProc
    command: [root.masterControlPath, "status"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.updateMasterState(text)
    }
  }

  Process {
    id: masterToggleProc
    command: [root.masterControlPath, "toggle"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.updateMasterState(text)
    }
  ...
  Process {
    id: sinkToggleProc
    command: [root.masterControlPath, "sink-toggle"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.updateMasterState(text)
    }
  ...
  Process {
    id: sourceToggleProc
    command: [root.masterControlPath, "source-toggle"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.updateMasterState(text)
    }
  ...
  Timer {
    ...
    onTriggered: root.refreshMasterState()
  }
      ...
      ScrollView {
        ...
        Column {
          ...
          Item {
            ...
            // header's only cursor target. Its state comes from the live audio
            // server so stale Quickshell nodes cannot make it lie or no-op.
            ToggleSwitch {
              id: powerSwitch
              checked: root.masterAudible
              busy: masterToggleProc.running
