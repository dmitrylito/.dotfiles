...
metadata: 
  ...
  modified: 2026-07-23T05:36:51.167Z
---

Guidance layers on `dlco`, chezmoi-tracked where static:
- **`~/.claude/rules/`** (auto-loaded; chezmoi `dot_claude/rules/`): `code-style`, `chezmoi`, `system`, `memory` (always-on); `python`, `shell` (path-scoped via `paths:` frontmatter). The `memory` rule tells me to proactively save durable facts.
