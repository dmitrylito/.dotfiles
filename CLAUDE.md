...
- **`~/.codex/config.toml` is also a `modify_` target** (`dot_codex/modify_private_config.toml.tmpl`). Codex writes project trust, hook hashes, and desktop state into this file at runtime. The encrypted seed lives at `scripts/codex-config-baseline.toml.age`; the modifier fills missing baseline keys while preserving all existing live values. Update the encrypted seed deliberately for fresh machines, not to capture routine runtime churn.
